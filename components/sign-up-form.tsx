              ...
              <form className="flex flex-col gap-5" onSubmit={handleSignUp}>
                ...
                <div className="mt-8 flex gap-4">
                  <button
                    ...
                    onClick={() => setStep(1)}
                    disabled={isLoading}
                    className="flex h-[50px] w-[110px] items-center justify-center gap-2 rounded-[10px] border border-white/10 bg-white/5 text-[0.95rem] font-medium text-white transition-all hover:bg-white/10 hover:border-white/20 disabled:opacity-50"
                  >
                  ...
                  >
                    <div className="absolute -left-full top-0 h-full w-full bg-gradient-to-r from-transparent via-white/20 to-transparent transition-all duration-500 group-hover:left-[100%]" />
                    <span>{isLoading ? "Synchronizing..." : "Complete"}</span>
                    {!isLoading && <CheckCircle2 className="h-[1.1rem] w-[1.1rem]" />}
                  </button>
                </div>
